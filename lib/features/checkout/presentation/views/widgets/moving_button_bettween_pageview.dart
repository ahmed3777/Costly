...
class MovingButtonBettweenPageView extends StatelessWidget {
  const MovingButtonBettweenPageView(
      {super.key,
              ...
              onPressed: () {
                if (currentPageIndex == 2) {
                  if (paymentMethod == "paypal") {
                    _paypalProcessPayment(context);
                  } else if (paymentMethod == "creditcard") {
                    // مثلًا: فتح شاشة تأكيد أو عرض رسالة
                    showErrorBar(context, "سيتم الدفع بالبطاقة عند التسليم.");
                  } else {
                    showErrorBar(context, "الرجاء اختيار طريقة دفع.");
                  }
                } else {
  ...
  void _paypalProcessPayment(BuildContext context) {
    PaypalPaymentEntity paypalPaymentEntity =
        PaypalPaymentEntity.fromEntity(cart);
    ...
    Navigator.of(context).push(MaterialPageRoute(
      builder: (context) => PaypalCheckoutView(
        ...
        onSuccess: (Map params) async {
          //ايتم ارسال البيانات الى الباك اند و الذهاب الي تاكيد الدفع
